...
export function Experience() {
  return (
    <section id="experience" aria-labelledby="experience-heading" className="border-t border-border bg-surface py-16 sm:py-24">
      <Container>
        <SectionHeading id="experience-heading" number="03" title="Professional experience" />
        <div className="relative mt-10 before:absolute before:bottom-8 before:left-[5px] before:top-8 before:w-px before:bg-border">
          {experience.map((item, index) => (
            <article key={item.company} className="relative grid gap-5 border-t border-border py-8 pl-8 first:border-t-0 first:pt-0 md:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)] md:gap-12">

...
export function ArchitectureDiagram({
  ...
  return (
    <figure className={`overflow-hidden rounded-lg border border-diagram-border bg-surface-secondary text-foreground ${compact ? "p-4" : "p-5 sm:p-6"}`}>
      <figcaption className="mb-5 flex items-center gap-3 font-mono text-xs font-medium uppercase leading-5 tracking-[0.08em] text-secondary">
      ...
      <ol aria-label={caption} className="flex flex-col sm:flex-row sm:items-stretch">
        {steps.map((step, index) => (
          <li key={step} className="flex min-w-0 flex-1 flex-col items-center sm:flex-row">
            <span className={`relative w-full rounded-md border border-diagram-border bg-surface text-center font-mono text-xs font-medium leading-4 text-foreground shadow-[0_4px_12px_-10px_rgba(23,32,51,0.7)] ${compact ? "px-2 py-2.5" : "px-2 py-3.5"}`}>
